...
It's often frustrating with tutorials and quickstarts that show you how to draw stick figures and then follow that up with the Sistene Chappel so this is an attempt to provide a couple of examples to bridge the gap between the two and hopefully they will help you get some air under your wings so you can get started quickly and focus on shaping things from there. 
...
In another folder called `yamlgenerator`, there is an html file that you can download and use with any browser that has a form with a live updating code block side by side for the ECK managed Elasticsearch object. Download it and explore how and where the options fit into the larger YAML.
